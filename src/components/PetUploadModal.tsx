...
export const PetUploadModal: React.FC<PetUploadModalProps> = ({
  ...
  return (
    <div 
      id="modal-pet-upload"
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-150 font-sans"
    >
      <div className="bg-[#FDFCF8] border border-[#DAD2C3] rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[92vh]">
        
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-[#E5E0D5] flex items-center justify-between bg-[#F5F2EA]/60">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[#3A5A40]/15 flex items-center justify-center text-[#3A5A40]">
              <Heart className="w-4 h-4 text-[#3A5A40] fill-[#3A5A40]/20" />
            </div>
            <div>
              <h2 className="text-base font-serif font-bold text-[#3A5A40]">
                Submit Your Pet
              </h2>
          ...
          <button
            ...
            className="w-8 h-8 rounded-xl hover:bg-[#EBE7DF] text-[#7A746B] hover:text-[#4A453E] flex items-center justify-center transition cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          {approvedPet ? (
            ...
            <div className="space-y-4 text-center py-2 animate-in zoom-in-95 duration-200">
              <div className="relative mx-auto w-36 h-36 rounded-2xl overflow-hidden border-2 border-[#3A5A40] shadow-md">
                <img 
                  src={approvedPet.imageUrl} 
                  alt={approvedPet.petName}
                  className="w-full h-full object-cover" 
                />
                <div className="absolute top-1 right-1 bg-[#3A5A40] text-[#FDFCF8] p-1 rounded-full shadow-xs">
                  <CheckCircle2 className="w-4 h-4" />
                </div>
              </div>

              <div className="space-y-2">
                <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-[#3A5A40]/15 text-[#3A5A40] text-xs font-bold border border-[#3A5A40]/30">
                  <Sparkles className="w-3.5 h-3.5 text-[#B08968]" />
                  {approvedPet.petName} is approved!
                </div>
                
                <h3 className="text-xl font-serif font-bold text-[#3A5A40]">
              ...
              <div className="pt-2">
                <button
                  ...
                  type="button"
                  onClick={handleCloseModal}
                  className="w-full py-2.5 px-4 rounded-xl bg-[#3A5A40] hover:bg-[#2C4430] text-[#FDFCF8] font-bold text-xs shadow-md transition cursor-pointer"
                >
                  Close
                </button>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-[#4A453E]">
                  Pet Photo <span className="text-[#B08968]">*</span>
                </label>
                ...
                {imagePreview ? (
                  <div className="relative group rounded-2xl overflow-hidden border-2 border-[#DAD2C3] bg-[#F5F2EA] flex items-center justify-center h-48">
                    <img 
                      src={imagePreview} 
                      alt="Pet preview" 
                      className="w-full h-full object-cover"
                  ...
                  <div
                    ...
                    onDrop={handleDrop}
                    onClick={() => fileInputRef.current?.click()}
                    className="border-2 border-dashed border-[#DAD2C3] hover:border-[#3A5A40] bg-[#F5F2EA]/50 hover:bg-[#F5F2EA] rounded-2xl p-6 text-center cursor-pointer transition flex flex-col items-center justify-center gap-2"
                  >
                    {isProcessingImage ? (
                      <Loader2 className="w-8 h-8 text-[#3A5A40] animate-spin" />
                    ) : (
                      <div className="w-12 h-12 rounded-2xl bg-[#3A5A40]/10 flex items-center justify-center text-[#3A5A40]">
                        <Camera className="w-6 h-6" />
                      </div>
                    )}
                    <div>
                      <div className="text-xs font-bold text-[#3A5A40]">
              ...
              <div className="space-y-1">
                <label className="block text-xs font-bold text-[#4A453E]">
                  Pet's Name <span className="text-[#B08968]">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 text-xs bg-[#FDFCF8] border border-[#DAD2C3] rounded-xl focus:border-[#3A5A40] focus:ring-1 focus:ring-[#3A5A40] outline-none text-[#4A453E]"
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="block text-xs font-bold text-[#4A453E]">
                  Short Story / Note <span className="text-[#7A746B] font-normal">(optional)</span>
                </label>
                <textarea
                  rows={2}
                  value={description}
                  ...
                  className="w-full px-3 py-2 text-xs bg-[#FDFCF8] border border-[#DAD2C3] rounded-xl focus:border-[#3A5A40] focus:ring-1 focus:ring-[#3A5A40] outline-none text-[#4A453E] resize-none"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-[#4A453E] flex items-center gap-1">
                    <MapPin className="w-3 h-3 text-[#7E8260]" />
                    Location <span className="text-[#7A746B] font-normal">(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={location}
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-[#4A453E] flex items-center gap-1">
                    <UserIcon className="w-3 h-3 text-[#7E8260]" />
                    Your Name <span className="text-[#7A746B] font-normal">(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={submitterName}
              ...
              <div className="p-3 bg-[#F5F2EA] rounded-xl border border-[#DAD2C3] text-[11px] text-[#7A746B] leading-relaxed">
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-2xl font-bold text-xs shadow-md transition flex items-center justify-center gap-2 ${
                    isAiReviewing || !imagePreview || !petName.trim()
                      ? 'bg-[#DAD2C3] text-[#7A746B] cursor-not-allowed'
                      : 'bg-[#3A5A40] hover:bg-[#2C4430] text-[#FDFCF8] cursor-pointer'
                  }`}
                >
                  {isAiReviewing ? (
                    <>
